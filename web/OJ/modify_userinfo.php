...
if (isset($_POST['admin_mode']) && HAS_PRI("edit_user_profile")) {
    ...
    if (
        get_order(get_group($_SESSION['user_id'])) < get_order(get_group($user_id)) //权限比他高
        ...
        // 更新课程班级
        $course_team = $_POST['course_team'];
        $sql = "DELETE FROM course_team_relation WHERE user_id='$user_id'";
        $mysqli->query($sql);

        foreach ($course_team as $team_id) {
            $team_id = $mysqli->real_escape_string($team_id);
            $sql = "INSERT INTO course_team_relation (user_id, team_id) VALUES ('$user_id', '$team_id')";
            $mysqli->query($sql);
        }
